...
/*=========================================================
        MULTI-CLASS DATA ROUTING
=========================================================*/

const CLASS_CONFIG = {
    class6: { label: "Class 6" },
    class7: { label: "Class 7" },
    class8: { label: "Class 8" },
    class9: { label: "Class 9" },
    class10: { label: "Class 10" }
};

let activeClassId =
    localStorage.getItem("scienceAcademyClass") || "class9";

if(!CLASS_CONFIG[activeClassId]){
    activeClassId = "class9";
}

/*
    Class 9 keeps the original localStorage keys.
    Other classes receive their own localStorage namespace.
*/
function classStorageKey(baseKey){
    if(activeClassId === "class9"){
        return baseKey;
    }

    return baseKey + "_" + activeClassId;
}

function classDataRef(name){

    /* Class 9 remains on its original live data. */
    if(activeClassId === "class9"){
        return doc(db,"academy",name);
    }

    /* Other classes are fully isolated under their own namespace. */
    return doc(db,"classes",activeClassId,"data",name);
}

/*=========================================================
        CLASS SWITCH PASSWORDS
=========================================================*/
...
function switchClassWithPassword(classId) {

    // Invalid class protection
    if (!CLASS_CONFIG[classId]) {
    ...
    // Logged-in admin can switch classes without entering
    // the class password.
    ...
    // STUDENT / NORMAL USER PASSWORD CHECK
    const password = prompt(
        `Enter the password for ${CLASS_CONFIG[classId].label}:`
    );
    ...
    if (password === CLASS_PASSWORDS[classId]) {
        ...
        alert("❌ Incorrect class password.");
    }
}
...
async function loadData(name){
    const snap = await getDoc(classDataRef(name));
...
if(!currentUser){
    ...
    currentUser = prompt(
        "Enter your name for " +
        CLASS_CONFIG[activeClassId].label + ":"
    );
...
const dashboardClassSelect =
    document.getElementById("dashboardClassSelect");

const activeClassBadge =
    document.getElementById("activeClassBadge");
...
function updateClassUI(){
    ...
    if(dashboardClassSelect){
        dashboardClassSelect.value = activeClassId;
    }

    if(activeClassBadge){
        activeClassBadge.textContent =
            label.toUpperCase();
    }

    document.title =
        "Science Hack Academy | " + label;
}



async function reloadForActiveClass(){
    ...
    chatUnsubscribe = startRealtimeChat();
}

async function switchClass(classId){

    if(
        !CLASS_CONFIG[classId] ||
    ...
    if(!currentUser){
        ...
        localStorage.setItem(
            "scienceUser_" + activeClassId,
            currentUser.trim() || "student"
        );
    }

    await reloadForActiveClass();
...
/*=========================================================
        PROTECTED CLASS SWITCHING
=========================================================*/

function requestClassAccess(classId){
...
if(classSelect){
    classSelect.addEventListener(
        ...
        e => {
            ...
            if(requestClassAccess(requestedClass)){
                switchClass(requestedClass);
            } else {
                e.target.value = activeClassId;
            }
...
if(dashboardClassSelect){
    dashboardClassSelect.addEventListener(
        "change",
        e => {
...
/*
    No admin password is stored in this JavaScript file.
    Firebase Authentication performs the credential check.
*/
...
if(resetWebsiteBtn){
    ...
    resetWebsiteBtn.onclick = ()=>{
            ...
            const belongsToCurrentClass =
                activeClassId === "class9"
                    ? (
                        ...
                        key === "academyChat" ||
                        key === "academyStats" ||
                        key.startsWith("scienceBox")
                    )
                    : key.endsWith("_" + activeClassId);
        ...
        notify(
            "Current Class Data Reset Successfully"
        );
...
const key=classStorageKey("scienceBox"+index);
...
localStorage.getItem(classStorageKey("academyChat"))
) || [];

function saveChat(){

localStorage.setItem(
classStorageKey("academyChat"),
...
localStorage.getItem(classStorageKey("scienceBox0")),

tests:

localStorage.getItem(classStorageKey("scienceBox1")),

competition:

localStorage.getItem(classStorageKey("scienceBox2")),

records:

localStorage.getItem(classStorageKey("scienceBox3")),

announcement:

localStorage.getItem(classStorageKey("scienceBox4")),

notes:

localStorage.getItem(classStorageKey("scienceBox5"))
...
let academyStats = JSON.parse(
    localStorage.getItem(classStorageKey("academyStats"))
) || {
...
let syllabusData =
JSON.parse(
    localStorage.getItem(classStorageKey("academySyllabus"))
)
||
{
    text: ""
...
localStorage.getItem(classStorageKey("academySettings"))
...
localStorage.getItem(classStorageKey("academyTeacher"))
